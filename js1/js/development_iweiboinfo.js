tpl.iweiboinfo = [
	...
	'<div class="deverLeft">',
		...
		'<%}else if (app.app_status==1 ) {%>',
			...
			'<a href="javascript:;" class="hidebtn closealert">收起↑</a>',
			'<h4>你的组件已可用</h4>',
			'<div class="alert_content">如需显示网站来源，可将组件<a href="/development/iweibocheckuser?appid=<%=app.app_id%>" >提交审核</a>审核通过后，来源显示自动生效</div>',
			'</li>',
		...
		'<%} else if (app.app_status==7) {%>',
			...
			'<div class="alert_content"><%=app.app_check_msg%><br/>你可以修改后重新<a href="/development/iweibocheckuser?appid=<%=app.app_id%>">提交审核</a>，如有疑问请<a href="http://wiki.open.t.qq.com/index.php/%E8%81%94%E7%B3%BB%E6%88%91%E4%BB%AC" target="_blank">联系我们</a></div>',
			'</li>',
