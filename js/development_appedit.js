;(function(){
				...
				'<tr>',
					'<td valign="bottom" align="left" height="75">',
						...
						'<label for="icon1" class="uploadbtn">',
							'上传',
						'</label>',
						'<input type="file" _size="20" accept="image/png" name="icon1" id="icon1" data-default="<%=app.app_icon_16%>" <%if (app.app_type ==6 ) {%>class="required" _size="15"<%} else {%>class="file"<%}%>  data-label="<%if (app.app_type == 5) {%>组件图标<%} else {%>应用图标<%}%>"/>',
					...
					'<td valign="bottom" align="left">',
						...
						'<label for="icon2" class="uploadbtn">',
							'上传',
						'</label>',
						'<input type="file" _size="20" accept="image/png" name="icon2" id="icon2" data-default="<%=app.app_icon_50%>" <%if (app.app_type ==6 ) {%>class="required" _size="15"<%} else {%>class="file"<%}%> data-label="<%if (app.app_type == 5) {%>组件图标<%} else {%>应用图标<%}%>"/>',
					'</td>',
					'<td valign="bottom" align="left">',
						'<label class="nullimg">',
						...
						'<label for="icon3" class="uploadbtn">',
							'上传',
						'</label>',
						'<input type="file" _size="20" accept="image/png" name="icon3" id="icon3" data-default="<%=app.app_icon_64%>" <%if (app.app_type ==6 ) {%>class="required" _size="15"<%} else {%>class="file"<%}%> data-label="<%if (app.app_type == 5) {%>组件图标<%} else {%>应用图标<%}%>"/>',
					'</td>',
					'<td valign="bottom" align="left">',
						'<label class="nullimg">',
						...
						'<label for="icon4" class="uploadbtn">',
							'上传',
						'</label>',
						'<input type="file" _size="20" accept="image/png" name="icon4" id="icon4" data-default="<%=app.app_icon_75%>" <%if (app.app_type ==6 ) {%>class="required" _size="15"<%} else {%>class="file"<%}%> data-label="<%if (app.app_type == 5) {%>组件图标<%} else {%>应用图标<%}%>"/>',
					'</td>',
					'<%if (app.app_type ==6 ) {%>',
						'<td valign="bottom" align="left">',
						'<label class="nullimg">',
						...
						'<label for="icon5" class="uploadbtn">',
							'上传',
						'</label>',
						'<input type="file" _size="20" accept="image/png" name="icon5" id="icon5" data-default="<%=app.app_icon_114%>" <%if (app.app_type ==6 ) {%>class="required" _size="15"<%} else {%>class="file"<%}%> data-label="<%if (app.app_type == 5) {%>组件图标<%} else {%>应用图标<%}%>"/>',
					'</td>',
					'<%}%>',
				'</tr>',
				'<tr>',
	...
	'<div id="content" class="controlCon main main_app">',
		...
		'<h1 class="comp_tit">应用基本信息</h1>',
			...
			'<form action="/development/saveappinfopage?appid=<%=app.app_id%>" method="post" class="appform <%if (app.app_type==6) {%>wirelessappform<%}%>" enctype="multipart/form-data" id="appform" target="appform_post_aec" onsubmit="return false">',
			'<ul>',
				'<%if (app.app_status==2) {%>',
