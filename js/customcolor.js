$(function(){
	$("input[name='color']").change(function(){
		if(!$("#use_customcolorset").attr('checked')){
			$('#customcolor').hide();
			$('#autocolor').hide();
		}else{
			$('#customcolor').show();
	...
	var getPreviousEl = function(fromel,nodename){
		if( fromel.previousSibling.nodeName.toLowerCase() !== nodename){
			return getPreviousEl(fromel.previousSibling,nodename);
		}
		return fromel.previousSibling;
	};
	var getNextEl = function(fromel,nodename){
		if( fromel.nextSibling.nodeName.toLowerCase() !== nodename){
			return getNextEl(fromel.nextSibling,nodename);
		}
		return fromel.nextSibling;
	}
	//color Picker
	$("#customcolor>li>div").click(function(){
		var colorselector,
			el = $(this).get(0);
		colorselector =	getPreviousEl(el,'input') 
		colorselector.focus();
	});
	var colorCombos = 
	["#097054 #FFDE00 #6599FF #FF9900",
	"#000080 #FFFFFF #000000 #C0C0C0",
	"#993300 #99CC99 #003366",
	"#00628B #E6E6DC #81A594",
	"#666633 #CCCC99 #FFFFFF #990033",
	...
	"#CCCCCC #003366 #FFFFFF #CCCC99",
	"#FFFF66 #FFCC00 #FF9900 #FF0000",
	"#443266 #C3C3E5 #F1F0FF #8C489F",
	"#CC0000 #99FF00 #FFCC00 #3333FF",
	"#0099CC #CCFFCC #66CCFF #003399",
	"#333399 #3399FF #FFFFFF #3366CC",
	...
	"#D01A55 #FCB3D0 #585947 #E8E6DA #CCCEA9 #A6AF7A",
	...
	"#004489 #E1E1D6 #D3D9DF #989898 #565656 #DBDBCE",
	...
	"#C5CDD8 #A5B557 #841F27 #354E71",
	"#FF8F00 #5A8F29 #3C7DC4 #2B2B2B",
	"#FFCC33 #FF6600 #A45209 #533419",
	"#FEFCD7 #A6CB45 #71B238 #6A8347",
	...
	"#668E39 #96B566 #BCE27F #7C7C7C #C3C3C3 #F6FF97",
	"#421C52 #732C7B #9C8AA5 #BDAEC6 #FFFFFF",
	"#BED661 #89E894 #78D5E3 #7AF5F5 #34DDDD #93E2D5",
	"#206BA4 #BBD9EE #EBF4FA #C0C0C0 #E7E4D3 #F1EFE2",
	...
	"#52ADDA #68B8DF #C9C9C9 #DBDBDB #AACD4B #B5D954",
	"#595F23 #829F53 #A2B964 #5F1E02 #E15417 #FCF141",
	"#424242 #C5BD99 #FFFFFF #701112",
	"#3D577A #677C97 #E3E6EB #AEBD63 #8F2831",
	...
	"#092E20 #234F32 #326342 #92CC47 #9AEF3F #FFE761",
	"#0D3257 #97A7B7 #B2C2B9 #BDD6E0 #65754D #C7DBA9",
	"#31659C #639ACE #94BAE7 #4A79A5 #FFFFFF",
	...
	"#000000 #272B2E #3B4043 #666699 #BEF243",
	...
	"#287AA9 #3087B4 #143D55 #70822E #A0B842 #999999",
	...
	"#79B837 #9DCE5C #C7EB6E #E5E5E5 #444041 #0A0A0A",
	...
	"#73AF59 #FFFFFF #000000",
	"#D72729 #FBB601 #1B6AA5",
	"#1A2828 #28AF63 #C2D98B",
	"#7D868F #000000 #5A6F8E",
	"#D5331E #FFD900",
	"#D5331E #FFFFFF #6078D8",
	"#4C50A9 #F4A817",
	"#3964C3 #000000 #D34328",
	"#02243C #005DB3 #5195CE #E6E6E6 #F9F9F9 #5BC236",
	"#927B51 #A89166 #80C31C #BCDD5A #FF7900 #FBB36B",
	"#757116 #AEBC21 #D9DB56 #00477F #4C88BE #8DC3E9",
	"#B96A9A #D889B8 #9CC089 #D8F3C9 #FDE8D7 #FFFFFF",
	"#194E84 #3B6B9C #1F242A #37414A #60BB22 #F2BABB",
	...
	"#616536 #A4E666 #31B96E #1B1112",
	"#E850A8 #D0AC11 #F5F1DE #000316"];

	var applyColorCombo = function(i){
		var colorCombo = colorCombos[i].split(" ");
		if(!colorCombo){ return; }
		$("#customcolor>li>input").each(function(eli,el){
			var colorValue = "#000000";//边框黑色
			if(colorCombo[eli]){
				colorValue = colorCombo[eli];
			}
			el.value = colorValue;
			getNextEl(el,'div').style.background = colorValue;
		})
		if ($('#customcolor').css('display') == 'block')
		{
			crUrl('', 4);
		}
	}
	...
	$('#autocolor').click(function(){
		applyColorCombo(random(0,colorCombos.length))			
	});

	$('#autocolor').trigger('click');

	jscolor.onConfirmChange( function(){
		crUrl('', 4);
	} )
})
