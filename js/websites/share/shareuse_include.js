...
var appkey = '',
shareid = "share_btn_" + (new Date().getTime());
...
function bico() {
	var _img, x = parseInt($('input[name=t1]:checked').val()),
	y = parseInt($('input[name=size]:checked').val()),
	z = $("#qshareable").is(":checked") ? "": " onclick=\"postToWb();\"";
	if (x == 1) {
		$("#replytext").hide();
...
function doscript() {
	var _img = bico(),
	qshareable = $("#qshareable").is(":checked"),
	//是否绑定Q-Share
	compresscode = $("#compresscode").is(":checked"),
	//是否压缩代码
	...
	if (!checkappkey(appkey)) {
		...
		codestr = codestr.replace("$assname$", $("#assname").val()).replace("$appkey$", appkey).replace("$share_btn$", shareid);
		if (qshareable) {
