...
//<!-- <li><a href="/development/compdel/222">删除组件</a></li> -->
	...
	'<a href="javascript:;" class="hidebtn closealert">收起↑</a>', 
	'<h4>你的组件已可用</h4>', 
	'<div class="alert_content">如需显示网站来源，可将组件 <a href="/development/compcheckuser?comp_id=<%=comp.comp_id%>" class="a1">提交审核</a>', 
	'审核通过后，来源显示自动生效</div>', '</li>', 
	'<%}else if(comp.comp_status==2){%>', 
	'<li class="alert alert_warn">', '<h4>组件审核中</h4>', 
	'</li>', '<%}else if(comp.comp_status==7){%>', 
	'<li class="alert alert_warn">', 
	'<a href="javascript:;" class="hidebtn closealert">收起↑</a>', 
	'<h4>你的组件未通过审核，原因如下：</h4>', '<div class="alert_content"><%=comp.comp_check_msg%><br/>', 
	'你可以修改后重新<a href="/development/compcheckuser?comp_id=<%=comp.comp_id%>" class="a1">提交审核</a>，如有疑问请<a href="http://wiki.open.t.qq.com/index.php/%E8%81%94%E7%B3%BB%E6%88%91%E4%BB%AC" class="a1" target="_blank">联系我们</a></div>', 
	'</li>', 
	'<%}else if(comp.comp_status==3){%>', '<li class="alert alert_warn">', 
	'<a href="javascript:;" class="hidebtn closealert"></a>', 
	'<h4>你的组件已通过审核</h4>', '</li>', '<%}else{%>', 
	'<li class="alert alert_warn">', 
	'<a href="javascript:;" class="hidebtn closealert">收起↑</a>', 
	'<h4>你的组件为非法状态</h4>', 
	'<div class="alert_content" class="a1">如有疑问请<a href="http://wiki.open.t.qq.com/index.php/%E8%81%94%E7%B3%BB%E6%88%91%E4%BB%AC" target="_blank">联系我们</a></div>', '</li>', '<%}%>', '<%if(comp.comp_bd_change){%>', 
	...
	'</li>', '<%}%>', 
	'<li><strong>网站名称：</strong><em><%=comp.comp_name%>&nbsp; &nbsp;<a href="/development/compsite?comp_id=<%=comp.comp_id%>">修改</a></em></li>', '<li><strong>网站地址：</strong><em><a href="<%=comp.comp_url%>" target="_blank" class="a1"><%=comp.comp_url%></a>&nbsp; <a href="/development/compsite?comp_id=<%=comp.comp_id%>">修改</a></em></li>', 
	' <li><strong>组件类型：</strong><em><%=comp.comp_type_name%>&nbsp; &nbsp;<a href="/websites/<%if(comp.comp_type==1||comp.comp_type==7){%>share<%}else if(comp.comp_type==2){%>followcomp<%}else if(comp.comp_type==3){%>wall<%}else if(comp.comp_type==4){%>qshare<%}else if(comp.comp_type==5){%>mood<%}else if(comp.comp_type==6){%>comment<%}else if(comp.comp_type==8){%>login<%}else if(comp.comp_type==9){%>read<%}%>/" target="_blank">详细介绍</a></em></li>', 
...
function compType1() {
	...
	if (comp.comp_style) {
		var comp_style = comp.comp_style ? comp.comp_style: {
			"version": 2,
			"iconindex": 1,
			"showcounter": 1,
			"counterpos": "top"
		...
		var codestr = '<div class="code_tit">1. 将以下代码插入到页面中需要部署分享按钮的地方</div>' + '<div contenteditAble="true" class="code">' + ['&lt;div id="qqwb_share__" data-appkey="' + comp.comp_id + '" ', 'data-icon="' + share_iconindex + '" ', 'data-counter="' + share_showcounter + '" ', share_showcounter ? 'data-counter_pos="' + share_counterpos + '" ': '', 'data-content="{title}" ', richable && 'data-richcontent="{line1}|{line2}|{line3}" ' || '', 'data-pic="{pic}"&gt;&lt;/div&gt;'].join('').replace(/\{(\w+)\}/g, '<span class="c_red">{$1}</span>') + '</div>' + '<div class="code_tit">2. 将以下代码插入到<span class="c_green">&lt;/body&gt;</span>之前</div>' + '<div contenteditAble="true" class="code">' + '&lt;script src="http://mat1.gtimg.com/app/openjs/openjs.js#autoboot=no&debug=no"&gt;&lt;\/script&gt;' + '</div>' + '<div class="code_tit">3. 把代码中标红的参数按以下说明进行相应替换</div>' + '<table width="100%" cellpadding="7" border="1" class="paratable">' + '<tr><th>title</th><td>默认的文本内容或Rich化分享时的消息体标题，rich化时最多15个全角字符长度</td></tr>' + (function(richable) {
			if (richable === 0) {
				return '';
			} else {
				return '<tr><th>line1</th><td>Rich化消息体第一行文字，最多15个全角字符长度</td></tr>' + '<tr><th>line2</th><td>Rich化消息体第二行文字，最多15个全角字符长度</td></tr>' + '<tr><th>line3</th><td>Rich化消息体第三行文字，最多15个全角字符长度</td></tr>';
			}
		})(richable) + '<tr><th>pic</th><td>需要被分享的图片url，多张图片以|分隔</td></tr>' + '</table>';
		...
		*/
		...
		comp_style = comp.comp_style ? comp.comp_style: {
			"btnstyle": 1,
			"btnsize": 1,
			"btntext": "分享到腾讯微博",
			"assname": "api_weibo",
		...
		qsharebtn = ["w:118;h:25;b:share/qshare.png;", "w:92;h:22;b:qshare/icon3.gif;", "w:92;h:22;b:qshare/icon.gif;", "w:59;h:22;b:qshare/icon2.png;", "w:32;h:32;b:qshare/icon3.png;", "w:28;h:28;b:qshare/icon4.png;", "w:63;h:25;b:qshare/icon5.png;", "w:68;h:23;b:qshare/icon6.png;"];
		...
		function compressCode(compressed) {
			if (comp_type == 1) {
				...
				if ($("#outlink").is(":checked")) {
					...
					codestr = codestr.replace("$assname$", comp_style.assname).replace("$appkey$", comp_id).replace("$share_btn$", sharebtn).replace("$css$", qsharebtn[comp_style.qsharestyle].replace(/w:(\d+)?;h:(\d+)?;b:([^;]*)?;/g, "width:$1px;height:$2px;background:url(http://mat1.gtimg.com/app/opent/images/websites/$3);"));
				}
...
function compType3() {
	var comp_style = comp.comp_style ? comp.comp_style: {
		"topicnames": "%E7%BE%8E%E5%A5%B3,%E6%B8%94%E6%B0%B4%E4%B9%8B%E6%AC%A2,%E7%A6%8F%E5%88%A9",
		"width": 300,
		"height": 550,
		"autowidth": true,
		"colorstyle": 1,
		"defaultcolorstyle": 4,
		"customcolor": "B9FF40_FFF0D1_6A437A_63FFBC_91FFCF_54FF82",
		"imgshow": 0,
		"postpos": 0
	};
	...
	function getCodestr() {
		var _t = [],
		cus = comp_style.colorstyle * 4,
		_tn = comp_style.topicnames.replace(/,/g, "|");
...
function compType4() {
	...
	var comp_id = comp.comp_id;
	var qsharebtn = ["w:118;h:25;b:share/qshare.png;", "w:92;h:22;b:qshare/icon3.gif;", "w:92;h:22;b:qshare/icon.gif;", "w:59;h:22;b:qshare/icon2.png;", "w:32;h:32;b:qshare/icon3.png;", "w:28;h:28;b:qshare/icon4.png;", "w:63;h:25;b:qshare/icon5.png;", "w:68;h:23;b:qshare/icon6.png;"];
...
function compType6() { //<!--微评论-->
	var comp_style = comp.comp_style ? comp.comp_style: {
		...
		"colorstyle": 1,
		"defaultcolorstyle": -1,
		"customcolor": "white_white_white_white"
	}
	var comp_id = comp.comp_id;
	function getCodestr() {
