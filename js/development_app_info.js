	...
	'<!--客户端应用-->',
		...
		'<li>',
			'<label class="form_label"><em>*</em>',
				'应用类型：',
			'</label>',
			'<div class="form_element">pc客户端应用</div>',
		'</li>',
		'<li showfor="app_for_pc">',
			'<label class="form_label">',
				'<em>*</em>应用网址：',
	...
	//	'<li style="height:82px;">',
		...
		'</div>',*/
		 '<div id="app_support_list">',
		   ' <dl>',
		    	'<dd>',
			    	'<input type="checkbox" name="check_app_os"  id="app_pc_0" data-rule="isempty" value="Windows" <%if(/Windows/.test(app.appOs)){%>checked<%}%>/><label for="app_pc_0">Windows</label>', 
			    	'<input type="checkbox" name="check_app_os"  id="app_pc_1" data-rule="isempty" value="Mac" <%if(/Mac/.test(app.appOs)){%>checked<%}%>/><label for="app_pc_1">Mac</label>', 
			    	'<input type="checkbox" name="check_app_os"  id="app_pc_2" data-rule="isempty" value="Linux" <%if(/Linux/.test(app.appOs)){%>checked<%}%>/><label for="app_pc_2">Linux</label>',
				'</dd>',
		    '</dl>',
		'</div>',
...
tpl.apptype4 = [
	'<!--站内应用-->',
		...
		'<li>',
			...
			'<input type="radio" id="app_hosting" name="app_hosting" value="1" <% if(app.app_hosting  || app.app_status ==3 ||app.app_status ==5||app.app_status ==8||app.app_status ==6||app.app_status ==4) {%>  <%if (app.app_hosting) {%> checked="true"  <% } %> disabled= "disabled" <% } %> />',
			...
			'<a href="javascript:;" id="tuguan" class="valign">托管/非托管详细介绍</a>',
				'<div style="margin-top:-10px;_ margin-top:0;">',
					'<input type="radio" name="app_hosting"id="app_nohosting" value="0"  <% if(app.app_hosting || app.app_status ==3 ||app.app_status ==5||app.app_status ==8||app.app_status ==6||app.app_status ==4) { %> disabled= "disabled"  <%if (app.app_hosting ) {%>  <%} else {%> checked="true" <% } %><%} else {%> checked="true" <% } %>/>',
