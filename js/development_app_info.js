	...
	'<%} else if (app.app_type ==3) {%>',
		...
		'<li>',
			...
			'<select name="app_class_main" id="app_class_main" data-rule="app_class_main" data-error="请选择分类" class="app_class_main">',
				...
				'<% for (var i = 0, typelist; typelist = typelist.2[i]; i++) {%>',
	...
	'<%} else if (app.app_type ==6) {%>',
		...
		'<li>',
			...
			'<span class="form_element form_element2 form_checkplatform">',
				'<input type="checkbox" class="appplatform" name="appplatform" id="iphone_pf" <%if (app.app_platform==3 or $app.app_platform==1) {%>checked<% } %> <%if (iphoneinfo.app_plat_status ==2) {%>disabled<% } %>/><label for="iphone_pf" class="checkbox_label">iPhone</label>',
				'<input type="checkbox" class="appplatform" name="appplatform" id="android_pf" <%if (app.app_platform==3 or $app.app_platform==2) {%>checked<% } %> <%if (androidinfo.app_plat_status ==2) {%>disabled<% } %>/><label for="android_pf" class="checkbox_label">Android</label>',
		...
		'<li>',
			...
			'<span class="form_element form_selectclass">',
				...
				'<select name="app_class_child" id="app_class_child" data-rule="app_class_child" data-error="请选择分类" >',
					...
					'<% for (var i = 0, typelist; typelist = typelist_1.1; i++) {%>',
						'<%if (app.app_class_child == $typelist_1.cid) {%>',
							'<option value="<%=typelist_1.cid%>" selected="selected"><%=typelist_1.cname%></option>',
						'<% } else { %>',
							'<option value="<%=typelist_1.cid%>"><%=typelist_1.cname%></option>',
						'<% } %>',
					'<% } %>',
				'<% } else { %>',
	...
	'<%} else {%>',
		...
		'<li>',
			...
			'<span class="form_element form_selectclass">',
				'<select name="app_class_main" id="app_class_main" data-rule="app_class_main" data-error="请选择分类"  class="app_class_main">',
				'<option value="-1">请选择</option>',
				'<% for (var i = 0, typelist; typelist = typelist.0; i++) {%>',
					'<%if (app.app_class_child== $typelist.cid) {%>',
					'<option value="<%=typelist.cid%>" selected="selected"><%=typelist.cname%></option>',
					'<% } else { %>',
					'<option value="<%=typelist.cid%>"><%=typelist.cname%></option>',
					'<% } %>',
