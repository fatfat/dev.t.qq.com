	...
	'var _share_tencent_weibo = function() {',
		'var share_btn = function(_arr) {',
			...
			'else {',
				...
				'_ostyle = "$css$position:absolute;display:none;";',
		...
		'var _site = ""; //你的网站地址',
		'var _appkey = "$appkey$" || "801000271"; //你从腾讯微博开放平台获得的appkey',
		'var _web = {',
			'"name": document.title || "",',
			'"href": location.href.replace(/([^\x00-\xff]+)/g,encodeURIComponent("$1")),',
			'"hash": location.hash,',
			'"target": "toolbar=0,status=0,width=630,height=530"',
		'};',
		'var _pic = function(area) {',
			...
			'return _srcarr.join("|");',
		'}; //图片地址',
		'var _u = "http://share.v.t.qq.com/index.php?c=share&a=index&f=q1&url=$url$&appkey=" + _appkey + "&assname=$assname$&title=$title$&pic=$pic$";',
		...
		'document.onmouseup = function(e) {',
			...
			'for (var i = 0; i < share_area.length; i++) {',
				'if (share_area[i].contains(o) || share_area[i] == o) {',
					...
					'if (_select() && _select().length >= 10 && o != share_btn) {',
		...
		'document.onmouseover = function(e) {',
			'var curtarget = (e && e.target) || (window.event && window.event.srcElement),',
			'sx = parseInt(share_btn.style.width),',
			'sy = parseInt(share_btn.style.height),',
			'd = Math.min(sx, sy);',
			'if (curtarget.tagName.toLowerCase() == "img") {',
				...
				'if (curtarget.clientWidth >= 150 && curtarget.clientHeight >= 150) {',
					'show(share_btn, erect.right - sx - d, erect.bottom + document.body.scrollTop + document.documentElement.scrollTop - sy - d);',
					'share_btn.setAttribute("shareimg", curtarget.src);',
				'}',
			'} else if (curtarget != share_btn && share_btn.getAttribute("shareimg")) {',
...
/*
_share_tencent_weibo(qshare_btn,qshare_area);
qshare_btn:Q-Share功能按钮，选中文字或鼠标放在功能区域中的大图上后显示的转播按钮
qshare_area:Q-Share功能区域，可以是一个Dom数组
如须自定义功能，qshare_btn,share_area自己设置,其中qshare_area可以是一个DOM数组
*/