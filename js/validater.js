...
OPEN_VALIDATOR = {
	...
	cardnumIsSingle: function(value, selector) {
		...
		if (true === OPEN_VALIDATOR.cardnum(value)) {
			$.getJSON('/developer/checkid/' + encodeURIComponent(value),
			function(d) {
				//d = $.parseJSON(d);
				if (d.error == 0) {
					showmsg(true, selector, '');
	...
	compname: function(value, selector) { //应用全称
		var k, tchar, msg = selector && selector.attr("data-error") || "网站名称",
...
$("form input[type='submit']").click(function() { //表单提交验证
	...
	if (submitrule == 'formauto') { //表单自动提交
		...
		data = '',
		onlyworker = $(f).find("input[data-only]"),
		imgisok = true;
...
var SELECTADDR = {
	initAddr: function() {
		...
		//加载省份下拉框
		var arr = [];
		for (var p in LOCATION[1]) { ("n" != p) && arr.push('<li _value="' + p + '">' + LOCATION[1][p].n + '</li>');
		}
	...
	bindEvents: function(input) {
		...
		input.attr("readonly", "true").click(function() {
			input.trigger("focus");
		}).focus(function() {
...
$(document).ready(function() {
	...
	$(".form_select_city").find("li").live("click",
	function(event) {
	...
	$(".form_select_options").find("li").live("mouseover",
	function() {
...
var upload_op_type = 0,
//标志上传和更改：上传0 更改1
upload_idcard_suc = 0,
upload_certifpic_suc = 0; //上传成功标志位
//删除图片
$(".delete_pic").live("click",
function() {
...
$("input[type='file']").change(function() {
	...
	$this = $(this),
	pic_type = $this.siblings(".pic_type"),
...
var user_certif_status = global_obj.data.user_certif_status;
var submitCallback = function(d) {
	//var ret = +(d.ret||d.error),msg=common.getMsgByRet(ret);
	var ret = +d.ret || d.code,
	msg = common.getMsgByRet(ret);
	loginWin.close();
	if (msg) {
		loginWin.alert("<center>" + msg + "</center>");
	...
	if (ret == 0) {
		if (loginWin) {
			...
			if (user_certif_status === 1 && (user_check_status === 0 || user_check_status === 3)) {
				...
				function() {
					location.href = "/development/developer/";
					return false;
				},
				function() {
					location.reload();
