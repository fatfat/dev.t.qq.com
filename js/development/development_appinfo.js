...
//	    '<!-- 审核结果提示 -->',
	    ...
	    '<h4>你的应用未通过审核，原因如下：</h4>',
	    '<div class="alert_content"><%=app.app_check_msg%><br />',
	    '你可以修改应用后<a  <%if(app.app_checkapi == 0 && app.app_type == 4){%> href="javascript:;" id="sumbit_appcheckdeveloper2"   <%}else{%> href="/development/appcheckdeveloper/<%=app.app_id%>/" <%}%>>重新提交审核</a>，如有疑问，<a href="http://wiki.open.t.qq.com/index.php/%E8%81%94%E7%B3%BB%E6%88%91%E4%BB%AC" target="_blank">请联系我们</a></div>',
	    '</li>',
	...
	'<%}else{%>',
	   ...
	   '<li><strong>来源显示：</strong><em><%=app.source_status_display%></em><em',
	    ...
	    '<%if (app.app_status == 1 || app.app_status == 7){%>',
	        '<a <%if (app.app_checkapi == 0 && app.app_type == 4){%> href="javascript:;" id="sumbit_appcheckdeveloper" <%}else{%> href="/development/appcheckdeveloper/<%=app.app_id%>/" <%}%> class="btn4 <%if (6 == app.app_type){%> wirelessapp_btn4<%}%>">提交审核</a>', 
	    '<%}else{%>', 
...
(function ($) {
 	$('body').html(tmpl(developer_appinfo,data));
...
$(function(){  
 ...
 var app_nosubmit = '<!--{$app.app_nosubmit}-->';
  var androidinfo_app_plat_status = '<!--{$androidinfo.app_plat_status}-->';
    var iphoneinfo_app_plat_status = '<!--{$iphoneinfo.app_plat_status}-->';
  var app_check_status ='<!--{$app.app_check_status}-->'; 
	...
	$(".appinfo li.alert").find(".hidebtn").click(function(){
        ...
        if (p.attr("beclose")){
        p.hide();
        $.get("/development/clearappcheckmsg/<!--{$app.app_id}-->?t="+new Date().getTime());
        return;
        }
        if (c.is(":visible")){
            c.slideUp("fast");
...
function beforeCommitCheck(appplatform){
    var options = $(".appstate_des").first().find(":checkbox"),nextURL="/development/appcheckdeveloper/<!--{$app.app_id}-->",unchecked = options.not(":checked");
    if (unchecked.size()>0){
...
$(function(){
    ...
    $('a#change_type,a#change_type_2').click(function(){
    	if(app_status == 1||app_status == 7||app_status==5||app_status==4||app_status==8){
            ...
            $sureBtn.bind("click", function () {
                ...
                $.ajax({
                    type: "POST",
                    url: "/development/saveappinfo/"+appid+'/',
                    dataType: "json",
                    data: postData,
                    success: function(msg){
                        if(msg.error == 0){
                            if(app_status==1||app_status==7){
                            	...
                            	loginWin.close();
                                loginWin.alert('<center>已提交，通过审核后才能生效。</center>',function(){window.location.href="/development/appedit/"+appid+'/'});
                            }   
                         }else if(msg.error == 1){  
                          	 loginWin.close();
 ...
 function apptostore(appplatform){
     ...
     $.ajax({
          url: '/development/apptostore/'+appid,
          dataType: "json",
          data:postData,
          type:"POST",
          cache: false,
          success: function(ResposeData){  
            if(ResposeData.ret == 0){
				...
				loginWin.confirm({'text':'<center>基本信息填写完整后才能申请上架<br />现在去填写基本信息?</center>','ok_text':'确定','cancel_text':'取消'},function(){location.href='/development/appedit/<!--{$app.app_id}-->';},function(){loginWin.close();});
            }else if(ResposeData.ret < -300 && appplatform){
            	var platform = appplatform == 1 ? 'iPhone' : "Android" ;
				loginWin.confirm({'text':'<center>' +  platform + '平台信息填写完整后才能申请上架<br />现在去填写平台信息?</center>','ok_text':'确定','cancel_text':'取消'},function(){location.href='/development/platforminfo/<!--{$app.app_id}-->';},function(){loginWin.close();});
            }else{
