;(function(){
	...
	tpl.comp_area = [
		...
		'</dl>',
		'</div>',
		'<script type="text/javascript" src="http://mat1.gtimg.com/app/opent/js/comp_validate.js?20120523"></script>',
	].join("");
	...
	$('#main').html(tmpl(tpl.explain_include, data));	
...
/*
	...
	function crUrl(o){
		...
		var _t = [],cus=$("input[name='color']:checked").val()*4,_colorStyle=$('#colorList li.s').attr('data-color') - 0 || 4;
		if($('#t1').size() && $('#t1').val() != ''){
...
function formSubmit(){
    ...
    var paras={
    		"comp_type":3, //组件类型 1、'一键分享',2'批量收听',3'话题墙',4'Q-Share',5'心情板'
