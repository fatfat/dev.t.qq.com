...
tpl.websites_read_explain_include = [
	...
	'<div class="panel">',
	 '<h4>来源设置：</h4>', 
	 '<span class="form_input"><input type="text" name="SourceUrl" size="36" maxlength="60" data-rule="applink" data-error="来源URL" placeholder="指定一个发表微博后会自动带上的url地址" value="http://dev.t.qq.com/websites/read/"/></span>', 
	 '</div>', 
	 '</li>', 
	 '<li>', 
		'<div class="panel">', 
			...
			'<span class="form_input"><input type="text" name="InitialContent" size="36"  maxlength="140" data-rule="wordTip" data-error="默认展示文字" placeholder="指定发表框默认显示的内容" value="#阅读墙测试# 说点什么吧"/></span>', 
		'</div>', 
	'</li>', 
	'</ul>',
...
function makeFilter() {
	...
	for (var i in keyWords) {
		filter.keyWords.push(keyWords[i]);
	}
	return filter;
}
	...
	var compConfig = {
		...
		"TimelineModuleConfigure": (function(lis) {
			...
			lis.each(function() {
				var conf = {},
				li = $(this);
				conf.Condition = li.attr("Condition").replace(/\t+/g,"\t").split("\t");
...
eventBindFuncList.push(function() {
	...
	$("#configboard").find("input").change(function() {
		...
		if ($(this).attr("name") == "width" || $(this).attr("name") == "height") {
