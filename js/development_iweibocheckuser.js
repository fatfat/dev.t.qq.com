	...
	tpl.iweibocheckuser = [	
		...
		'<div id="content" class="controlCon main main_app">',
			...
			'<div class="deverRight"> ',
				...
				'<div class="app_check">',
					'<strong>确认开发者信息</strong><i></i><label>确认组件信息</label><i></i><label>确认素材</label><i></i><label>提交审核</label>',
				'</div>',
				'<!--开发者信息-->',
				'<form action=" " method="post" class="appform"  id="appform_user"  onsubmit="return false;" >',
				'<ul class="appinfo_certif">',
					'<% if (developer_detail.user_type !=2 ) {%>',
						'<input type="hidden" name="user_type" id="user_type" value="1" />',
						'<li>',
							'<label class="form_label"><em>*</em>姓名：</label><span class="form_input"><input type="text" class="txt" name="user_name" id="user_name" value="<%=developer_detail.user_name%>" data-default="<%=developer_detail.user_name%>" data-rule="comperson" data-error="姓名" maxlength="30"></span>',
						'</li>',
						'<li style="height:61px;">',
							'<label class="form_label"><em>*</em>个人身份证件：</label><span class="form_input form_select">	',
								'<input type="text" name="user_id_type" id="user_id_type"',
									'value="<%if (developer_detail.user_id_type == "1"){%>身份证<%} else if (developer_detail.user_id_type == "0"){%>护照<%} else {%>身份证<% } %>" ',
									'_value="<%if (developer_detail.user_id_type == "1") {%>1<%} else if (developer_detail.user_id_type == "0") {%>0<%} else {%>1<% } %>" ',
									'data-default="<%if (developer_detail.user_id_type == "1") { %>1<%} else if (developer_detail.user_id_type == "0") {%>0<%} else {%>1<% } %>" ',
									'style="width:102px"/>',
								...
								'</span>',
										'<div class="form_div"><span class="form_input">',
											...
											'placeholder="<%if (developer_detail.user_id_type == "1") {%>身份证号码<%}else if (developer_detail.user_id_type == "0"){%>护照号码<%}else{%>身份证号码<%}%>"',
											'data-rule="<%if (developer_detail.user_id_type == "1"){%>cardnum_new<%}else if (developer_detail.user_id_type == "0"){%>passport<%}else{%>cardnum_new<%}%>" ',
											'data-error="<%if (developer_detail.user_id_type == "1"){%>身份证号码<%}else if (developer_detail.user_id_type == "0") {%>护照号码<%}else{%>身份证号码<%}%>"',
											'data-default="<%=developer_detail.user_id_card_num%>" value="<%=developer_detail.user_id_card_num%>" maxlength="30"/></span>',
										'</span></div>',
									'</li>',
									'<li style="height:61px;">',
										...
										'</span>',
										'<div class="form_div"><span class="form_input"><input type="text" class="txt" name="user_address" id="user_address" value="<%=developer_detail.user_address%>" data-rule="comaddress" data-error="联系地址" maxlength="100"></span></div>',
									'</li>',
									'<li>',
										'<label class="form_label"><em>*</em>手机号码：</label><span class="form_input"><input type="text" class="txt" name="user_phone" id="user_phone"  value="<%=developer_detail.user_phone%>" data-rule="mobilenum" data-error="手机号码"  maxlength="11"></span><cite class="gray inputdes">仅限中国境内手机号码，无需加0或+86</cite>',
									'</li>',
									'<li> ',
										'<label class="form_label">QQ：</label><span class="form_element form_element2"><%=userInfo.uin%></span>',
									'</li>',
									'<li>',
										'<label class="form_label">网站地址：</label><span class="form_input"><input type="text" class="txt" name="user_website" id="user_website" value="<%=developer_detail.user_website%>" data-rule="applink" data-error="网站地址"></span>',
									'</li>',
		'<%} else {%>',
		'<input type="hidden" name="user_type" id="user_type" value="2" />',
								'<li>',
										'<label class="form_label"><em>*</em>公司全称：</label><span class="form_input"><input type="text" class="txt" name="user_company" id="user_company" value="<%=developer_detail.user_company%>" data-default="<%=developer_detail.user_company%>" data-rule="companyname" data-error="公司全称" maxlength="50"></span>',
									'</li>',
									'<li>',
										'<label class="form_label"><em>*</em>营业执照号码：</label><span class="form_input"><input type="text" class="txt" name="user_business_code" id="user_business_code" value="<%=developer_detail.user_business_code%>" data-rule="complicensenum" data-error="营业执照号码" data-default="<%=developer_detail.user_business_code%>"  maxlength="20"></span>',
									'</li>',
									'<li style="height:61px;">',
										...
										'<span class="form_input form_select" style="margin-left:12px;">',
												...
												'<ul class="form_select_options form_select_options_certif form_select_city"></ul>',
										'</span>',
										'<div class="form_div"><span class="form_input"><input type="text" class="txt" name="user_address" id="user_address" value="<%=developer_detail.user_address%>" data-rule="comaddress" data-error="公司地址" maxlength="100"></span></div>',
									'</li>',
									'<li>',
										'<label class="form_label"><em>*</em>公司电话：</label><span class="form_input"><input type="text" class="txt" name="user_tel" id="user_tel" value="<%=developer_detail.user_tel%>" data-rule="telnum" data-error="公司电话" placeholder="区号-电话号码-分机号"></span>',
